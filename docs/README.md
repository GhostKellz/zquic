...
ZQUIC provides a modular QUIC (RFC 9000) and HTTP/3 (RFC 9114) implementation in pure Zig with:

- **Modular builds**: Core-only (~1.3 MB) to full-stack (~5.5 MB)
- **Stable crypto**: AES-GCM, ChaCha20-Poly1305, X25519, Ed25519 via zcrypto v1.0.4
- **Experimental PQ crypto**: ML-KEM hybrid key exchange and ML-DSA-65 auth helpers (requires explicit flags)
...
### Integrations
- **[zcrypto](integrations/zcrypto.md)** - zcrypto v1.0.4 integration guide
...
### Guides
- **[Future Features](future-features.md)** - Scoped features intentionally deferred from the current release line
