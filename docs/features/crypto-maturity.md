...
This page describes the release posture of zquic crypto modules for v0.9.16.
...
| `src/crypto/comprehensive_tls.zig` | Bounded experimental engine | Exposes maturity markers, zcrypto-backed TLS ciphertext record parsing, encrypted TLS 1.3 record deprotection for supported AEAD suites, decrypted/plaintext handshake-message dispatch from parsed records, delegated zcrypto X.509 verification with caller-supplied roots, explicit certificate validation policy hooks, traffic-key/session-ticket helpers, key update helpers, EncryptedExtensions ALPN validation for `h3`, required EncryptedExtensions QUIC transport-parameter decoding, caller-supplied raw Ed25519 public-key verification, and transcript-bound raw Ed25519 CertificateVerify processing. It is still not a production TLS stack; external TLS interop vectors remain future work. |
...
| `src/crypto/comprehensive_tls.zig` | Parses TLS ciphertext record framing via `zcrypto.tls.record`; decrypts TLSInnerPlaintext for AES-128-GCM, AES-256-GCM, and ChaCha20-Poly1305 traffic keys after AEAD tag verification; dispatches decrypted/plaintext handshake records into the state machine; delegates X.509 DER parsing, hostname checks, and caller-supplied root validation to `zcrypto.tls.config`; exposes explicit production/raw-public-key/test certificate policy modes; validates EncryptedExtensions ALPN as `h3`; requires, decodes, and retains QUIC transport parameters from EncryptedExtensions; verifies caller-supplied raw Ed25519 public-key certificates with validity bounds and optional hostname/subject matching; verifies TLS 1.3 CertificateVerify signatures for raw Ed25519 certificates against the transcript hash; authenticates scaffold session tickets with HMAC; stores DER blobs as local containers and rejects local DER verification with `NotSupported`; rejects simplified RSA/ECDSA paths because `allow_simplified_verification` is false. | Complete certificate-chain policy, revocation posture, socket-fed transport-parameter policy in the live TLS path, SAN handling, and external TLS interop vectors. |
...
- Unsupported algorithms such as RSA, SLH-DSA, and X448 are out of the v0.9.16 crypto contract unless a later release implements and tests them.
