...
pub const Hkdf = struct {
    ...
    pub fn extract(comptime HashType: type, salt: []const u8, ikm: []const u8, prk: []u8) void {
        ...
        h.update(ikm);
        h.final(prk[0..hmac.mac_length]);
    }
    
    /// HKDF-Expand function
    pub fn expand(comptime HashType: type, prk: []const u8, info: []const u8, okm: []u8) !void {
        const hmac = crypto.auth.hmac.Hmac(HashType);
        const hash_len = HashType.digest_length;
        const n = (okm.len + hash_len - 1) / hash_len;
        
        if (n > 255) return Error.ZquicError.CryptoError;
        
        var t: [HashType.digest_length]u8 = undefined;
        var okm_offset: usize = 0;
        ...
        for (0..n) |i| {
            ...
            h.update(info);
            h.update(&[_]u8{@intCast(i + 1)});
            h.final(&t);
            
            const copy_len = @min(hash_len, okm.len - okm_offset);
            @memcpy(okm[okm_offset..okm_offset + copy_len], t[0..copy_len]);
            okm_offset += copy_len;
        }
    }
};
...
pub const EnhancedCryptoKeys = struct {
    ...
    pub fn deinit(self: *Self) void {
        ...
        crypto.utils.secureZero(u8, self.secret);
        crypto.utils.secureZero(u8, self.key);
        crypto.utils.secureZero(u8, self.iv);
        crypto.utils.secureZero(u8, self.header_protection_key);
...
pub const EnhancedAead = struct {
    ...
    pub fn encryptAesGcm(key: []const u8, iv: []const u8, plaintext: []const u8, aad: []const u8, allocator: std.mem.Allocator) ![]u8 {
        ...
        if (key.len == 16) {
            // AES-128-GCM
            const aes = crypto.aead.aes_gcm.Aes128Gcm;
            aes.encrypt(ciphertext[0..plaintext.len], ciphertext[plaintext.len..], plaintext, aad, iv[0..12].*, key[0..16].*);
        } else if (key.len == 32) {
            // AES-256-GCM
            const aes = crypto.aead.aes_gcm.Aes256Gcm;
            aes.encrypt(ciphertext[0..plaintext.len], ciphertext[plaintext.len..], plaintext, aad, iv[0..12].*, key[0..32].*);
        } else {
    ...
    pub fn decryptAesGcm(key: []const u8, iv: []const u8, ciphertext: []const u8, aad: []const u8, allocator: std.mem.Allocator) ![]u8 {
        ...
        if (key.len == 16) {
            // AES-128-GCM
            const aes = crypto.aead.aes_gcm.Aes128Gcm;
            aes.decrypt(plaintext, ciphertext[0..plaintext_len], ciphertext[plaintext_len..][0..16].*, aad, iv[0..12].*, key[0..16].*) catch {
            ...
            // AES-256-GCM
            const aes = crypto.aead.aes_gcm.Aes256Gcm;
            aes.decrypt(plaintext, ciphertext[0..plaintext_len], ciphertext[plaintext_len..][0..16].*, aad, iv[0..12].*, key[0..32].*) catch {
                allocator.free(plaintext);
                return Error.ZquicError.CryptoError;
            };
        } else {
    ...
    pub fn encryptChaCha20Poly1305(key: []const u8, iv: []const u8, plaintext: []const u8, aad: []const u8, allocator: std.mem.Allocator) ![]u8 {
        ...
        chacha.encrypt(ciphertext[0..plaintext.len], ciphertext[plaintext.len..], plaintext, aad, iv[0..12].*, key[0..32].*);
    ...
    pub fn decryptChaCha20Poly1305(key: []const u8, iv: []const u8, ciphertext: []const u8, aad: []const u8, allocator: std.mem.Allocator) ![]u8 {
        ...
        const chacha = crypto.aead.chacha_poly.ChaCha20Poly1305;
        chacha.decrypt(plaintext, ciphertext[0..plaintext_len], ciphertext[plaintext_len..][0..16].*, aad, iv[0..12].*, key[0..32].*) catch {
            allocator.free(plaintext);
            return Error.ZquicError.CryptoError;
        };
...
pub const EnhancedHeaderProtection = struct {
    ...
    pub fn generateAesMask(hp_key: []const u8, sample: []const u8) ![5]u8 {
        ...
        if (hp_key.len == 16) {
            // AES-128-ECB
            const aes = crypto.core.aes.Aes128;
            const ctx = aes.initEnc(hp_key[0..16].*);
            ctx.encrypt(&mask, sample[0..16]);
        } else if (hp_key.len == 32) {
            // AES-256-ECB
            const aes = crypto.core.aes.Aes256;
            const ctx = aes.initEnc(hp_key[0..32].*);
            ctx.encrypt(&mask, sample[0..16]);
        } else {
            return Error.ZquicError.CryptoError;
        }
    ...
    pub fn generateChaCha20Mask(hp_key: []const u8, sample: []const u8) ![5]u8 {
        ...
        var mask: [64]u8 = undefined; // ChaCha20 block size
        crypto.stream.chacha.ChaCha20IETF.xor(&mask, &mask, counter, hp_key[0..32].*, nonce);
...
pub const EnhancedTlsContext = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, is_server: bool, cipher_suite: EnhancedCipherSuite) !Self {
        ...
        crypto.random.bytes(&context.client_random);
        crypto.random.bytes(&context.server_random);
    ...
    pub fn encryptPacket(self: *const Self, level: EncryptionLevel, plaintext: []const u8, packet_number: u64, aad: []const u8) ![]u8 {
        ...
        return switch (self.cipher_suite) {
            .aes_128_gcm_sha256, .aes_256_gcm_sha384 => 
                try EnhancedAead.encryptAesGcm(keys.key, &nonce, plaintext, aad, self.allocator),
            .chacha20_poly1305_sha256 => 
                try EnhancedAead.encryptChaCha20Poly1305(keys.key, &nonce, plaintext, aad, self.allocator),
        };
    }
    ...
    pub fn decryptPacket(self: *const Self, level: EncryptionLevel, ciphertext: []const u8, packet_number: u64, aad: []const u8) ![]u8 {
        ...
        return switch (self.cipher_suite) {
            .aes_128_gcm_sha256, .aes_256_gcm_sha384 => 
                try EnhancedAead.decryptAesGcm(keys.key, &nonce, ciphertext, aad, self.allocator),
            .chacha20_poly1305_sha256 => 
                try EnhancedAead.decryptChaCha20Poly1305(keys.key, &nonce, ciphertext, aad, self.allocator),
        };
    }
