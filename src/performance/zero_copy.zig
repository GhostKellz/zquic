...
pub const HardwareCRC32 = struct {
    ...
    fn crc32Hardware(data: []const u8) u32 {
        ...
        while (i + 8 <= data.len) : (i += 8) {
            ...
            crc = @bitCast(asm volatile ("crc32 %[value], %[crc]"
                : [crc] "=r" (crc),
                : [value] "r" (value),
                  [crc] "0" (crc),
            ));
        ...
        while (i < data.len) : (i += 1) {
            crc = @bitCast(asm volatile ("crc32b %[byte], %[crc]"
                : [crc] "=r" (crc),
                : [byte] "r" (data[i]),
                  [crc] "0" (crc),
            ));
...
pub const ZeroCopyQuicProcessor = struct {
    ...
    pub fn processPacketBatch8(
        ...
        if (self.hw_caps.has_avx2 and self.simd_processor != null) {
            ...
            const results = try self.simd_processor.?.aes_gcm_encrypt_x8(packet_array, nonce_array, key_array);
...
pub const ZeroCopyPacketProcessor = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !Self {
        return Self{
            .buffer_pool = try BufferPool.init(allocator, 1024, 100),
            .crypto_accelerator = HardwareCrypto.init() catch null,
            .vectorized_io = VectorizedIO.init(allocator, &[_][]const u8{}),
            .allocator = allocator,
    ...
    pub fn deinit(self: *Self) void {
        ...
        self.vectorized_io.deinit(self.allocator);
    ...
    pub fn batchProcessPackets(self: *Self, raw_packets: [][]const u8) ![]ProcessedPacket {
        ...
        for (raw_packets, results) |packet_data, *result| {
            ...
            if (packet_data.len <= buffer.capacity) {
                ...
                const mapped_buffer = try ZeroCopyBuffer.initMapped(self.allocator, packet_data.len, try self.createTempFile(packet_data));
    ...
    pub fn processStreamData(self: *Self, stream_id: u64, data_chunks: [][]const u8) !StreamResult {
        ...
        for (data_chunks, result.buffers) |chunk, *buffer_ptr| {
            ...
            if (chunk.len > 4096) {
                ...
                const mapped_buffer = try ZeroCopyBuffer.initMapped(self.allocator, chunk.len, try self.createMappingForChunk(chunk));
