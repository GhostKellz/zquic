...
pub const CryptoAlert = struct {
    ...
    pub fn format(self: *const Self, comptime fmt: []const u8, options: std.fmt.FormatOptions, writer: anytype) !void {
        _ = fmt;
        _ = options;
        try writer.print("[{}] {} - {s}: {s} (current: {d:.2}, threshold: {d:.2})", .{ self.severity, self.timestamp, self.category, self.message, self.current_value, self.threshold_value });
    }
};
...
pub const LatencyHistogram = struct {
    ...
    pub fn init() Self {
        ...
        histogram.bucket_bounds = [_]u64{ 1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000, 200000, 500000, 1000000, 10000000 };
...
pub const CryptoTelemetrySystem = struct {
    ...
    pub fn deinit(self: *Self) void {
        ...
        self.alert_history.deinit(self.allocator);
    }
    ...
    pub fn collectMetrics(self: *Self) !void {
        ...
        // Add to history
        try self.metrics_history.append(self.allocator, self.current_metrics);
    ...
    fn checkThresholds(self: *Self) void {
        ...
        if (metrics.zero_rtt_success_rate < self.config.zero_rtt_success_rate_threshold) {
            self.raiseAlert(.warning, "Crypto", "Low Zero-RTT success rate", "zero_rtt_success_rate", metrics.zero_rtt_success_rate, self.config.zero_rtt_success_rate_threshold, "Check session ticket configuration and network conditions");
        }

        // Check packet loss rate
        if (metrics.packet_loss_rate > self.config.loss_rate_critical_threshold) {
            self.raiseAlert(.critical, "Network", "High packet loss rate", "packet_loss_rate", metrics.packet_loss_rate, self.config.loss_rate_critical_threshold, "Investigate network infrastructure");
        } else if (metrics.packet_loss_rate > self.config.loss_rate_warning_threshold) {
            self.raiseAlert(.warning, "Network", "Elevated packet loss rate", "packet_loss_rate", metrics.packet_loss_rate, self.config.loss_rate_warning_threshold, "Monitor network quality");
        }
        ...
        if (throughput_mbps < self.config.throughput_warning_threshold_mbps) {
            self.raiseAlert(.warning, "Performance", "Low throughput detected", "throughput_mbps", throughput_mbps, self.config.throughput_warning_threshold_mbps, "Check server capacity and network bandwidth");
        }
    }
    ...
    fn raiseAlert(self: *Self, severity: AlertSeverity, category: []const u8, message: []const u8, metric_name: []const u8, current_value: f64, threshold_value: f64, suggested_action: ?[]const u8) void {
        ...
        self.active_alerts.append(self.allocator, alert) catch |err| {
            std.log.err("Failed to add alert: {}", .{err});
            return;
        };

        self.alert_history.append(self.allocator, alert) catch |err| {
    ...
    pub fn exportJson(self: *const Self, writer: anytype) !void {
        ...
        try writer.print("\"connections\":{{\"total\":{},\"active\":{}}},", .{ self.current_metrics.total_connections, self.current_metrics.active_connections });

        // Performance metrics
        try writer.print("\"latency\":{{\"avg\":{},\"p50\":{},\"p95\":{},\"p99\":{}}},", .{ self.current_metrics.avg_latency_us, self.current_metrics.p50_latency_us, self.current_metrics.p95_latency_us, self.current_metrics.p99_latency_us });

        try writer.print("\"throughput\":{{\"bytes_per_sec\":{},\"requests_per_sec\":{:.2}}},", .{ self.current_metrics.bytes_per_second, self.current_metrics.requests_per_second });
        ...
        for (self.active_alerts.items, 0..) |alert, i| {
            ...
            try writer.print("{{\"id\":{},\"severity\":\"{}\",\"category\":\"{s}\",\"message\":\"{s}\",\"timestamp\":{}}}", .{ alert.id, alert.severity, alert.category, alert.message, alert.timestamp });
        }
        try writer.writeAll("]");
