...
pub const VpnNode = struct {
    ...
    fn connectToPeers(self: *Self) !void {
        for (self.config.peer_addresses, 0..) |peer_addr, i| {
            ...
            // Store connection
            try self.connections.append(allocator, connection);
    ...
    fn updateStatistics(self: *Self) void {
        ...
        if (self.load_balancer) |*lb| {
            ...
            std.log.debug("VPN Node Stats - Connections: {d}, Routes: {d}, LB Success Rate: {d:.2}%", .{ runtime_stats.active_connections, router_stats.route_count, lb_stats.success_rate * 100 });
        } else {
            std.log.debug("VPN Node Stats - Connections: {d}, Routes: {d}", .{ runtime_stats.active_connections, router_stats.route_count });
        }
    }
