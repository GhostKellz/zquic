...
const TradingOrder = struct {
    ...
    pub fn serialize(self: *const @This(), allocator: std.mem.Allocator) ![]u8 {
        return try std.fmt.allocPrint(allocator, "{{\"id\":{},\"symbol\":\"{s}\",\"type\":\"{}\",\"priority\":\"{}\",\"qty\":{d:.8},\"price\":{?d:.2},\"ts\":{}}}", .{ self.id, self.symbol, self.order_type, self.priority, self.quantity, self.price, self.timestamp });
    }
};
...
const MarketUpdate = struct {
    ...
    pub fn serialize(self: *const @This(), allocator: std.mem.Allocator) ![]u8 {
        return try std.fmt.allocPrint(allocator, "{{\"symbol\":\"{s}\",\"bid\":{d:.2},\"ask\":{d:.2},\"last\":{d:.2},\"vol24h\":{d:.2},\"ts\":{}}}", .{ self.symbol, self.bid, self.ask, self.last_price, self.volume_24h, self.timestamp });
    }
};
...
const CryptoTradingClient = struct {
    ...
    pub fn sendTradingOrder(self: *Self, order: TradingOrder) !void {
        ...
        const connection = try self.connection_multiplexer.acquireConnection(.custom, // Custom trading protocol
            conn_priority, .high_frequency_trading);
        ...
        const can_send = self.congestion_controller.canSend(@as(u32, @intCast(order_data.len)), order.priority == .emergency_liquidation or order.priority == .arbitrage);
        ...
        std.log.info("Sent {s} order for {s}: {} μs latency (Order ID: {})", .{ @tagName(order.order_type), order.symbol, latency, order.id });
    ...
    fn simulateOrderExecution(self: *Self, order: TradingOrder, send_latency: u64) !void {
        ...
        Time.sleep(execution_delay * 1000); // Convert to nanoseconds
    ...
    pub fn subscribeMarketData(self: *Self, symbols: []const []const u8) !void {
        ...
        const connection = try self.connection_multiplexer.acquireConnection(.http3, .high, .defi_api);
        defer self.connection_multiplexer.releaseConnection(connection);

        for (symbols) |symbol| {
            const subscription_data = try std.fmt.allocPrint(self.allocator, "{{\"action\":\"subscribe\",\"channel\":\"ticker\",\"symbol\":\"{s}\"}}", .{symbol});
    ...
    pub fn processMarketUpdate(self: *Self, update: MarketUpdate) !void {
        ...
        std.log.debug("Processed market update for {s}: {d:.2} @ {} μs", .{ update.symbol, update.last_price, processing_time });
    }
    ...
    pub fn runMonitoring(self: *Self) !void {
        while (true) {
            ...
            if (stats.orders_sent % 100 == 0 and stats.orders_sent > 0) {
                ...
                std.log.info("  Orders: {} sent, {} executed ({d:.1}% success rate)", .{ stats.orders_sent, stats.orders_executed, stats.execution_rate });
                std.log.info("  Latency: {} μs avg, {} μs execution", .{ stats.avg_latency_us, stats.avg_execution_time_us });
                std.log.info("  Volume: ${d:.2} USD", .{stats.total_volume_usd});
                std.log.info("  Connections: {} total, {d:.1} req/s", .{ stats.telemetry_summary.total_connections, stats.telemetry_summary.requests_per_second });
                std.log.info("  Network: {} μs p99 latency, {d:.1}% loss", .{ stats.telemetry_summary.p99_latency_us, stats.telemetry_summary.packet_loss_rate * 100 });
            }
...
pub fn runCryptoTradingDemo(allocator: std.mem.Allocator) !void {
    ...
    while (demo_timer.read() < demo_duration_seconds * std.time.ns_per_s) {
        ...
        // Control frequency
        Time.sleep(std.time.ns_per_s / orders_per_second);
